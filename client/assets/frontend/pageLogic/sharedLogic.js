function getKittyDna(genes) {
  try {
    if (genes.length != 16)
      throw `genes string ('${genes}') should be 16 characters (not ${genes.length})`;
...
function getSelectedCatIds(catIds) {
  try {
    ...
    for (i = 0; i < catIds.length; i++) {
      ...
      if ($(idCheckBox).prop("checked")) IdsSelectedCats.push(catIds[i]);
    }
...
function isNumberOfKitties(numRequired, numSelected, idErrorElement) {
  const kittyWord = numRequired == 1 ? "cat" : "cats";

  if (numSelected < numRequired) {
    $("#" + idErrorElement).text(
      `Use checkboxes to select ${numRequired} ${kittyWord}!`
    );
  ...
  if (numSelected > numRequired) {
    $("#" + idErrorElement).text(
      `Too many! Please select only ${numRequired} ${kittyWord}!`
    );
    $("#" + idErrorElement).css({ color: "red", "font-weight": "bold" });
    return false;
  }
