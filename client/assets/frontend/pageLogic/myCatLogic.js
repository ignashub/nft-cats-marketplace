...
$(document).ready(async function () {
  ...
  onMarketplaceEvent(updatemyCatsPage);
...
function updatemyCatsPage(newTx) {
  try {
    switch (newTx.TxType) {
      case "Create offer":
        console.log("In updatemyCatsPage(newTx): 'Create offer'");
        ...
        break;
      case "Buy":
        console.log("In updatemyCatsPage(newTx): 'Buy'");
        displayTransaction(newTx);
        ...
        break;
      case "Remove offer":
        console.log("In updatemyCatsPage(newTx): 'Remove offer'");
        displayTransaction(newTx);
    ...
    console.log("Error from updatemyCatsPage(newTx): " + error);
  }
}
...
function breeding() {
  try {
    ...
    window.location.href = `breed.html?firstCatId=${catIds[0]}&secondCatId=${catIds[1]}`;
  } catch (error) {
...
async function advertiseCat() {
  try {
    ...
    if (catOnSale) {
      $("#sellError").text(
        "Cat is already for sale! To relist at diferent price, please first withdraw current advertisement!"
      );
    ...
    $("#sellError").text(
      "Failed to create marketplace 'for sale' advertisement!"
    );
  }
}
...
async function removeAdvert() {
  try {
    ...
    if (!catOnSale) {
      $("#sellError").text(
        "There's no 'for sale' advertisement to remove for this kitty!"
      );
      $("#sellError").css({ color: "red", "font-weight": "bold" });
      return;
    }
    ...
    $("#sellError").text(
      "Failed to remove 'for sale' advertisement from the marketplace!"
    );
  }
}
