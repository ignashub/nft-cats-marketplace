...
const KITTY_CONTRACT_ADDRESS = "0xb71636B5252E1df7d8e0d6F2C0a7502a036202C9";
const MARKETPLACE_ADDRESS = "0x80007BF7b1163931B043ee80F5021152A96A1a5B";
...
async function initiateConnection() {
  try {
    ...
    Instance_Of_KittyContract = new web3.eth.Contract(
      abi.kittyContract,
      KITTY_CONTRACT_ADDRESS,
      { from: accounts[0] }
    );
    Instance_Of_Marketplace = new web3.eth.Contract(
      abi.marketplace,
      MARKETPLACE_ADDRESS,
      { from: accounts[0] }
    );
...
function isUser(address) {
  try {
    if (String(address).toLowerCase() !== String(User).toLowerCase())
      return false;
    return true;
  } catch (err) {
...
function onBirthEvent(uiCallbackFunc) {
  Instance_Of_KittyContract.events
    .Birth()
...
async function isOwnerOfKittyContract() {
  try {
    let isOwner;
    await Instance_Of_KittyContract.methods
      .getContractOwner()
      .call({}, function (err, contractOwner) {
        if (err) throw "Error from getContractOwner().call(): " + err;
        isOwner =
          String(contractOwner).toLowerCase() === String(User).toLowerCase();
      });
    return isOwner;
  } catch (error) {
...
function onMarketplaceEvent(uiCallbackFunc) {
  Instance_Of_Marketplace.events
    .MarketTransaction()
    .on("data", function (event) {
      uiCallbackFunc(event.returnValues);
    })
...
async function getAllCatIdsOnSale() {
  try {
    let catIdsOnSale = [];
    await Instance_Of_Marketplace.methods
      .getAllTokenOnSale()
      .call({}, function (err, idsTokensOnSale) {
...
async function getForSaleDetails(catId) {
  try {
    ...
    await Instance_Of_Marketplace.methods
      .getOffer(catId)
      .call({}, function (errMsg, offer) {
        if (errMsg) throw new Error(errMsg);
        if (catId != offer.tokenId)
          throw new Error(
            `Internal error - tokenId (${offer.tokenId}) returned by getOffer(catId) doesn't match catId (${catId})!?`
          );
...
async function setMarketplaceApproval() {
  try {
    const isMarketplaceAnOperator = await Instance_Of_KittyContract.methods
...
async function buyKitty(tokenId, priceInWei) {
  try {
    await Instance_Of_Marketplace.methods
      .buyKitty(tokenId)
      .send({ value: priceInWei }, function (err, txHash) {
        if (err) {
          throw err;
        } else {
          console.log(txHash);
